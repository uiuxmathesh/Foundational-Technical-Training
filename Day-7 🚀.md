...
FROM [Applicants]
		...
		INNER JOIN [Jobs] ON Jobs.JobID = Applications.JobID
		INNER JOIN [Companies] ON Companies.CompanyID = Jobs.CompanyID
...
SELECT Companies.CompanyID,Companies.CompanyName, COUNT(Jobs.JobID) AS Job_Count
FROM [Companies]
		INNER JOIN [Jobs] ON Jobs.CompanyID = Companies.CompanyID
GROUP BY Companies.CompanyID, Companies.CompanyName
HAVING COUNT(Jobs.JobID) = (
							SELECT TOP 1 COUNT(Jobs.JobID) AS JobID
							FROM [Companies]
									INNER JOIN [Jobs] ON Jobs.CompanyID = Companies.CompanyID
							GROUP BY Companies.CompanyID, Companies.CompanyName
							ORDER BY JobID DESC
...
SELECT Applicants.*,Jobs.JobTitle, Companies.*
FROM [Applicants] 
		INNER JOIN [Applications] ON Applicants.ApplicantID = Applications.ApplicantID
		INNER JOIN [Jobs] ON Jobs.JobID = Applications.JobID
		INNER JOIN [Companies] ON Companies.CompanyID = Jobs.CompanyID
...
FROM	[Companies]
		...
		LEFT JOIN [Applications] ON Applications.JobID = Jobs.JobID
GROUP BY Companies.CompanyID, Companies.CompanyName, Jobs.JobID
HAVING COUNT(Applications.ApplicationID) = 0
...
FROM [Applications]
		...
		RIGHT JOIN [Applicants] ON Applicants.ApplicantID = Applications.ApplicantID
...
FROM [Applicants]
	 ...
	 INNER JOIN [Jobs] ON Jobs.JobID = Applications.JobID
	 INNER JOIN [Companies] ON Companies.CompanyID = Jobs.CompanyID
WHERE Companies.Location = Applicants.City
